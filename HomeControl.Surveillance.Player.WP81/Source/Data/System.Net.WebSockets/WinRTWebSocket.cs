    ...
    internal class WinRTWebSocket
    {
        ...
        private void CancelAllOperations(WebSocketException customException)
        {
            ...
            if (_closeWebSocketReceiveResultTcs != null)
            {
                ...
                {
                    var exception = new WebSocketException(
                        "WebSocketError.InvalidState,SR.Format(SR.net_WebSockets_InvalidState_ClosedOrAborted,\"System.Net.WebSockets.InternalClientWebSocket\",\"Aborted\")");
            ...
            try
            {
                using (DataReader reader = args.GetDataReader())
                {
                    ...
                    while ((dataAvailable = reader.UnconsumedBufferLength) > 0)
                    {
                        ...
                        WebSocketReceiveResult recvResult = new WebSocketReceiveResult((int)readCount, messageType, endOfMessage);
